library(data.table)
library(ggplot2)
pileup_name_cov <- read.table("pileup_params.txt", stringsAsFactors=FALSE)
...
for (i in 1:dim(pileup_name_cov)[1]) {
  temp <- fread(pileup_name_cov[i,1], select = c(1:5),sep="\t")
  output_name <- paste(gsub(".pileup","_pileup.fasta",pileup_name_cov[i,1],fixed=TRUE))
  outputnameforseqname <- paste(gsub(".pileup","",pileup_name_cov[i,1],fixed=TRUE))
  tempseq <- NULL
  x <- 1
  temprec <- NULL 
  tempcov <- as.numeric(pileup_name_cov[i,2])
  for (j in 1:(dim(temp)[1])) {
    print(paste("Up to ",j," for sample ",i,sep=""))
    #1A what to do if coverage is above tempcov
    if (temp[j,4] > tempcov) {
      ...
      #2AB what to do for the rest of the rows when coverage is above tempcov
      } else {
        # 3A what to do when from same underlying reference fragment and coverage is above tempcov
        if (temp[j,1]==temp[(j-1),1]) {
          # 4A what to do when no sequence gap between rows and coverage is above tempcov
          if (temp[j,2]==(temp[(j-1),2]+1)) {
            ...
            tempseq <- basecall(j)
          # 4AB what to do if not sequential bases (start a new frag) and coverage is above tempcov
          } else {
            # 2000A if tempseq is not null
            if (!(is.null(tempseq))) {
              if (nchar(tempseq)>=100) {
